<?php
  ...
  if ($_SESSION['user'])
  {
    header('Location: my_account_s.php');
  }
...
<body style="background-color: rgb(235, 238, 247);">
  ...
  <div style="display: flex; justify-content: center; align-items: center; margin-top: 120px;">
    <form style="width: 400px;" method="post" action="../includes/registration.php">
      <div class="form-group">
        <label for="exampleInputText">ФИО</label>
        <input type="text" name="full_name" class="form-control" id="exampleInputText" aria-describedby="emailHelp" placeholder="Введите свое ФИО">
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        ...
        <input type="password" name="password1" class="form-control" id="exampleInputPassword1" placeholder="Введите пароль">
      </div>
      <div class="form-group">
        <label for="exampleInputPassword1">Подтверждение пароля</label>
        <input type="password" name="password2" class="form-control" id="exampleInputPassword1" placeholder="Подтвердите пароль">
      </div>
      <br>
      <button type="submit" class="btn btn-success" style="">Зарегистрироваться</button>
      <div class="login-links">
        <br>
        <p class="text-center">У Вас уже есть аккаунт? <a class="txt-brand" href="my_account.php"><font color=#29aafe>Войти</font></a></p>
        
        <?php
          if ($_SESSION['message']) {
            echo '<p class="message" style="border: 2px solid; color: orange; border-radius: 3px; padding: 10px; text-align: center; font-weight: bold;"> ' . $_SESSION['message'] . ' </p>';
          }
